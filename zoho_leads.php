...
if(!empty($zoho['refresh_token'])){
    ...
    if(!empty($response['access_token'])){
      ...
      // $url = 'https://www.zohoapis.com/crm/v2/settings/fields?module=Leads&scope=ZohoCRM.settings.fields.READ';
      // $ch = curl_init();
      ...
      foreach ($contacts as $row) {
          ...
          $sql = "select * from zoho_leads where ac_contact_id = '".$row['id']."' AND is_sent = 1";
          ...
          if(mysqli_num_rows($query) == 0){
              ...
              if(!empty($response2['data']['0']['code']) && $response2['data']['0']['code']=="SUCCESS"){
                $sql = "INSERT INTO zoho_leads (ac_contact_id,first_name,last_name,email,phone,zoho_response) VALUES ('".$row['id']."','".$row['first_name']."','".$row['last_name']."','".$row['email']."','".$row['phone']."','".json_encode($response2)."')";
